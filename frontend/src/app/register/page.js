      ...
      <div className="glass-card animate-in" style={{
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="form-label"><FiUser style={{ marginRight: '6px', verticalAlign: 'middle' }} /> Full Name</label>
            <input type="text" className="form-input" placeholder="Dr. Priya Sharma"
          ...
          <div className="form-group">
            <label className="form-label"><FiBriefcase style={{ marginRight: '6px', verticalAlign: 'middle' }} /> Role</label>
              ...
              {roles.map((r) => (
                <div
                  ...
                  style={{
                    ...
                    background: form.role === r.value ? 'rgba(6, 182, 212, 0.1)' : 'var(--bg-glass)',
                    transition: 'var(--transition)', textAlign: 'center',
                  }}
                >
                  <div style={{ fontSize: '1.3rem', marginBottom: '4px' }}>{r.label.split(' ')[0]}</div>
                  <div style={{ fontSize: '0.8rem', fontWeight: 600 }}>{r.label.split(' ').slice(1).join(' ')}</div>
                </div>
              ))}
            </div>
          </div>
          <button type="submit" className="btn btn-primary" disabled={loading}
            ...
            {loading ? <span className="spinner" style={{ width: '20px', height: '20px', borderWidth: '2px' }} /> : <><FiUserPlus /> Create Account</>}
          </button>
        </form>
